...
This program is an RMR Software 'badged' version of Ian Webster's Golf card game for the EPOC Series. 

As it stands, this Shareware version is fully-working with no disabled functions. However, after 30 days of use it will start to 'gently' remind you that it is time you registered, to support the Shareware principle and encourage us to provide you will more quality programs.

FILE DETAILS

The archive file should contain files:

RMRGolf.sis   The program
ReadMe.txt   English Instructions 
(plus other language README files)

Once installed. you will see the program consists of the following essential files: (all in /System/Apps/RMRGolf/)

RMRGolf.app     - The main program file
RMRGolf.mbm     - The icons used within the program
RMRGolf.aif     - The icons used in the System screen
RMRGolf.rsc     - The language resource file

And the following optional files which can be loaded as required:

RMRGolf.hlp     - Optional Help File

INSTALLATION

To install the program simply double-click on the RMRGolf.sis file or select the "Install New Program" option in PSIWIN (EPOC Install).
...
Start the program by selecting the 'Extras' from the Application bar and then 'RMRGolf'.

You can learn everything you need to about the program by playing with it and checking the effect of each MENU selection, particularly the 'Game preferences' and 'View preferences' options. We have not written any separate instructions, as we believe all the information you need is in the on-line Help file. 

All comments and, of course, registrations, gratefully received.

REGISTRATION

If you wish to register the program, you can do so by post (snail-mail), or on-line using  RegNet (credit cards).

If you are using the postal service, please send a cheque (check) (US, UK Sterling or Eurocheques only) for UKP £10 or US$15 or 30DM or 15 Euros payable to 'RMR Software' (or the equivalent in 'real money' in your actual currency) to one of the following addresses, including a stamped address envelope (or an E-Mail address) for the reply:
...
Please note these are the addresses of our 'agents'. If you use these addresses, Registration codes will be sent out fairly quickly, but if you send a technical query, it will have to be redirected to the author, so might take a few weeks to reply. If you have access, we can be reached quickly on support@rmrsoft.com

If you wish to register using a credit card you can register on the WWW through RegNet, the Registration Network. RegNet can be reached by e-mail: orders@swregnet.com or on the World Wide Web: http://www.swregnet.com or in the USA by calling 1-800-WWW2REG (1-800-999-2734) or the easiest way is by following the links from our WWW Home page at:

	http://www.rmrsoft.com/

This program has been allocated the REGNET Number: 5270 and this should be quoted. Again, the notification tends to get to us within 24 hours and we reply within another 24 hrs. (Note that paying by this method will involve an extra US$5 handling charge)

In each case, we (or our agents) will then send you a personalised registration code to enter to remove the 'nag' screens. We will also ensure that this code remains forward compatible for future versions of the program so, unlike other programs, we will not be asking for a further fee for an upgrade on the Series 5.
...
For the EPOC series of machines:
  ...
  RMRGolf      : Golf card game
  RMRMontana   : Montana card game
  ...
  RMRTalk     : A Talking Clock
  RMRShop     : A Shopping List Manager
  RMRAutotext : A Clipboard Manager
  Vactrac5    : A Leave/Vacation tracking program
  deVender    : The classic Defender arcade game
  SoundTrans  : A Sound convertor and player
