...
INTRODUCTION
------------
Welcome to our Vector Draw program for the EPOC Series of handheld computers.

As it stands, this Shareware version is unrestricted.  Please note that registration does not involve reloading the program, but simply selecting the 'Register' menu option and typing in a registration code to release the 'lock'.

FILE DETAILS
------------
The archive file should contain the following files:

  RMRDraw.sis  The program
  Readme.txt     English Instructions

Once installed you will see the program consists of the following essential files: (all in \System\Apps\RMRDraw\)

RMRDraw.app  - The main program file
RMRDraw.mbm  - The icons used within the program
RMRDraw.aif  - The icons used in the System screen
RMRDraw.rsc  - The Language resource file
D5Eng.dll    - Library file
D5View.dll   - Library file

And the following optional files which have been installed and may be deleted later:

RMRDraw.hlp - Help File

INSTALLATION
------------
To install the program simply double-click on the RMRDraw.sis file or select the "Install New Program" option in PsiWin/EPOC Install.

RUNNING THE PROGRAM
-------------------
Start the program by selecting 'Extras' from the Application bar and then selecting RMRDraw.

REGISTRATION
------------
If you wish to register the program, you can do so by post (snail-mail), or on-line using the RegNet facility (credit cards).

If you are using the postal service, please send a cheque (check) for 25 UK pounds, US$40, 75 DM  or 40 Euros payable to 'RMR Software' (or the equivalent in 'real money' in your actual currency - no foreign currency cheques please), to one of the following addresses, including a stamped address envelope (or an E-Mail address) for the reply:
...
This program has been allocated the RegNet Number: 6072 and this should be quoted. Again, the notification tends to get to us within 24 hours and we reply within another 24 hrs. (Note that paying by this method will involve an extra US$5 handling charge)
...
For the EPOC series of machines:
  ...
  RMRDraw       : A Vector Drawing program
