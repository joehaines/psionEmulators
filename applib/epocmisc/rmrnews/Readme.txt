RMRNews - Internet News Reader V1.02
------------------------------------
...
INTRODUCTION
------------
Welcome to our Internet News reader for the EPOC Series of handheld computers.

As it stands, this Shareware version is almost fully functional except it allows subscribtion to only three newsgroups for each server and doesn't support some of the menu options. Please note that registration does not involve reloading the program, but simply selecting the 'Register' menu option and typing in a registration code to release the 'lock'.

FILE DETAILS
------------
The archive file should contain the following files:

  RMRNews.sis  The program
  Readme.txt     English Instructions

Once installed you will see the program consists of the following essential files: (all in \System\Apps\RMRNews\)

RMRNews.app - The main program file
RMRNews.mbm - The icons used within the program
RMRNews.aif - The icons used in the System screen
RMRNews.rsc - The Language resource file

And the following optional files which have been installed and may be deleted later:

RMRNews.hlp - Help File

INSTALLATION
------------
To install the program simply double-click on the RMRNews.sis file or select the "Install New Program" option in PsiWin/EPOC Install.

RUNNING THE PROGRAM
-------------------
Start the program by selecting 'Extras' from the Application bar and then selecting RMRNews

Also, we strongly encourage you to read the Help file when setting up the program as it fairly complex.  Alternatively, FoxPop have produced a beginners tutorial at http://www.foxpop.ndirect.co.uk/rmrnews1.htm>

REGISTRATION
------------
If you wish to register the program, you can do so by post (snail-mail), or on-line using the RegNet facility (credit cards).

If you are using the postal service, please send a cheque (check) for 15 UK pounds, US$25, 50 DM  or 25 Euros payable to 'RMR Software' (or the equivalent in 'real money' in your actual currency - no foreign currency cheques please), to one of the following addresses, including a stamped address envelope (or an E-Mail address) for the reply:
...
Please note that these are the addresses of our 'agents'. If you use these addresses, Registration codes will be sent out fairly quickly, but if you send a technical query it will have to be redirected to us, so might take longer to reply. If you have access, we can be reached quickly at support@rmrsoft.com
...
This program has been allocated the RegNet Number: 5078 and this should be quoted. Again, the notification tends to get to us within 24 hours and we reply within another 24 hrs. (Note that paying by this method will involve an extra US$5 handling charge)
...
For the EPOC series of machines:
  ...
  RMRCrossword : Crossowrd Puzzle program
