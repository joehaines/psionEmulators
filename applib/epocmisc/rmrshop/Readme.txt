...
INTRODUCTION
------------
Welcome to our Shopping Trolley program for the EPOC Series of handheld computers.

As it stands, this Shareware version is fully functional but limits you to a few entries to enable you to try out the program before registering. Please note that registration does not involve reloading the program, but simply selecting the 'Register' menu option and typing in a registration code to release the 'lock'.

FILE DETAILS
------------
The archive file should contain the following files:

RMRShop.sis      The program
Readme.txt       English Instructions

Once installed. you will see the program consists of the following essential files: (All in the \System\Apps\RMRShop\ folder)

RMRShop.app	- The main program file
RMRShop.mbm	- The icons used within the program
RMRShop.aif	- The icons used in the System screen
RMRShop.rsc	- The Language resource file
RMRShop.def	- A default store

And the following optional file which you may have installed and can be deleted later:

RMRShop.hlp	- Help File
...
INSTALLATION
------------
To install the program simply double-click on the RMRShop.sis file or select the "Install New Program" option in PsiWin (EPOC Install). 

RUNNING THE PROGRAM
-------------------
Start the program by selecting 'Extras' from the Application bar and then selecting RMRShop.

REGISTRATION
------------
If you wish to register the program, you can do so by post (snail-mail), or on-line using the RegNet facility (credit cards).

If you are using the postal service, please send a cheque (check) for 10 UK pounds, US$15, 30 DM or 15 Euros payable to 'RMR Software' (or the equivalent in 'real money' in your actual currency - no foreign currency cheques please), to one of the following addresses, including a stamped address envelope (or an E-Mail address) for the reply:
