...
INTRODUCTION
------------
Welcome to our Home Inventory program for the EPOC Series of handheld computers.

As it stands, this Shareware version is fully functional but limits you to the number of rooms and items, to enable you to test the program before registering.  Please note that registration does not involve reloading the program, but simply selecting the 'Register' menu option and typing in a registration code to release the 'lock'.

FILE DETAILS
------------
The archive file should contain the following files:

RMRHome.sis    The program
Readme.txt     English Instructions

and other international Readme files

Once installed. you will see the program consists of the following essential files: (\System\Apps\RMRHome\)

RMRHome.app	- The main program file
RMRHome.mbm	- The icons used within the program
RMRHome.aif	- The icons used in the System screen
RMRHome.rsc	- The Language resource file

And the following optional files which you may have installed and can be deleted later:

RMRHome.hlp	- Help File
...
INSTALLATION
------------
To install the program simply double-click on the RMRHome.sis file or select the "Install New Program" option in PsiWin (EPOC Install). 

RUNNING THE PROGRAM
-------------------
Start the program by selecting 'Extras' from the Application bar and then selecting RMRHome.

REGISTRATION
------------
If you wish to register the program, you can do so by post (snail-mail), or on-line using the RegNet facility (credit cards).

If you are using the postal service, please send a cheque (check) for 10 UK pounds, US$15, 15 Euros or 30DM payable to 'RMR Software' (or the equivalent in 'real money' in your actual currency - no foreign currency cheques please), to one of the following addresses, including a stamped address envelope (or an E-Mail address) for the reply:
...
This program has been allocated the RegNet Number: 1326 and this should be quoted. Again, the notification tends to get to us within 24 hours and we reply within another 24 hrs. (Note that paying by this method will involve an extra US$5 handling charge)
