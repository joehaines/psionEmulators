...
INTRODUCTION
------------
RMRAutoText performs two functions: 1) It offers a way to store and quickly insert text, sketch, and other items that you use frequently. 2) It can also collect and paste multiple items.

RMRAutoText can be used in many tasks; inserting addresses, paragraphs, letterheads, e-mail addresses, and the list goes on. RMRAutoText reduces keystrokes reducing errors in your typing.

The following are just some of the features RMRAutoText provides:

- Create up to 999 entries per file.
- Create as many different files as you want.
- Text entries can be up to 5000 characters long.
- Insert time/date stamps into your documents using Plug-ins, or create your own plug-ins!
...
The archive file should contain the following files:
  ...
  RMRAutoText.sis  	The program
  netBook-Series7.zip   The colour version for the netBook and Series 7
  Readme.txt    	English Instructions
  Laesmig.txt    	Dutch Instructions

Once installed you will see the program consists of the following essential files: (all in \System\Apps\RMRAutoText\)

RMRAutoText.app - The main program file
RMRAutoText.mbm - The icons used within the program
RMRAutoText.aif - The icons used in the System screen
RMRAutoText.rsc - The Language resource file 

And the following optional files which have been installed and may be deleted later:

RMRAutoText.hlp - Help File

INSTALLATION
------------
To install the program simply double-click on the RMRAutoText.SIS or select the "Install New Program" option in PsiWin/EPOC Install.

RUNNING THE PROGRAM
-------------------
Start the program by selecting 'Extras' from the Application bar and then selecting RMRAutoText

REGISTRATION
------------
Benefits of registering are: 

- Unlimited number of entries (unregistered version limited to 5)
- No nag screens
- Free support if required

The registration is good for releases up to v2.99

If you wish to register the program, you can do so by post (snail-mail), or on-line using a credit card.  The registration fee is £12 or US$20 
If you wish to register on-line using a credit card you have 2 options:

USA Residents:

We now support the PayPal system for transferring of money. Simply send the money to alan@rmrsoft.com quoting the program you wish to register and we will send the registration code to you within a few hours.
...
This program has been allocated the RegNet Number: 5044 and this should be quoted.  The notification tends to get to us within 12 hours and we reply as soon as possible.
...
If you wish to register the program, you can do so by post (snail-mail), or on-line using the RegNet facility (credit cards).
