Title:		UKsights
Version:	99a February 99
Platform:	S3 and S5
Description:	Key details of interesting places to visit in the British 	
		Isles
...
These notes describe the 'UKsights' Psion database file, which provides 
...
'UKsights' consists of a single Psion database file. Two versions are 
provided depending on which type of Psion it will be loaded onto.

The filenames are:

1.  UKsights		(S5 version)

2.  Uksights.dbf	(S3a/S3c/Siena version)

The relevant file needs to be copied to the Psion without any conversion.
Typically using 'drag and drop' within the PsiWin application.
On the S5, a data file can be placed almost anywhere, whilst on the S3 it 
should be copied into the \DAT\ directory on any disk. The entries for the 
S5 version can be sorted into a different order than currently given,
however, at present they are in geographical order:
...
still underrepresented, but I hope to address that in later releases.
...
Geo:
An integer indicating geographical position. This field is hidden in the S5 
...
Shakespeare's  Birthplace '1564'. This field is hidden in the S5 version 
...
context. Only available on the S5 version.
...
http://
An Internet home page address if available.
...
information on your Psion. I don't expect any problems to arise but I like to
cover myself.
...
~ Indicates changed records since previous version
...
BT Tower
BT Museum of Telecommunications
